...
**Warning**: If no run configuration with a terminal icon appears, it might mean that your code has errors and CLion could not extract the executables from your program. If for some reason this situation persists, you can run the program using the green arrow next to the `main()` function, but make sure you first add the file to `CMakeLists.txt`, otherwise no flags will be applied to your program.
